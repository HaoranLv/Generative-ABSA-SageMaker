...
from transformers import AdamW, T5ForConditionalGeneration, T5Tokenizer,BertTokenizer
...
class CPTFineTuner(pl.LightningModule):
    def __init__(self, hparams):
        super(CPTFineTuner, self).__init__()
        self.hparams = hparams

        self.model = CPTForConditionalGeneration.from_pretrained(hparams.model_name_or_path)
...
def evaluate2(data_loader, model, paradigm, task, sents):
    """
    ...
    for i in range(len(targets)):
        gold_list = extract_spans_extraction('tasd-cn', targets[i])
        all_labels.append(gold_list)

    labels = np.array(all_labels)
    print(labels.shape)
    np.save('./labels_total.npy', labels)
    return raw_scores, fixed_scores


def evaluate(data_loader, model, paradigm, task, sents, hasidx=False):
    """
    Compute scores given the predictions and gold labels
    """
    device = torch.device(f'cuda:{args.n_gpu}')
    ...
    if args.do_eval:
        ...
        for checkpoint in all_checkpoints:
            ...
            if 0 <= int(epoch) < 100:
                ...
                dev_result,fix_res = evaluate(dev_loader, model, args.paradigm, args.task, sents)
